// Test script for CallGraph analysis
import path from 'path';
import { CallGraphService } from '../api/patterns/services/CallGraphService.js';
import fs from 'fs/promises';

async function testCallGraphAnalysis() {
  console.log('Starting callgraph analysis test...');
  ...
  try {
    // Run the analysis
    const callGraph = await CallGraphService.extractCallGraph(rootDir);
    ...
    console.log(`Found ${Object.keys(callGraph.functionGraph || {}).length} function dependencies`);
    ...
    console.log('---------------------------');
    Object.entries(callGraph.functionGraph || {}).forEach(([func, dependencies]) => {
    ...
    const updatedAnalyses = CallGraphService.updateRelatedFiles(mockAnalyses, callGraph);
